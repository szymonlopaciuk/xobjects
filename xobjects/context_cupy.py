...
class ContextCupy(XContext):
    ...
    def _build_module_with_clang(self, source, extra_compile_args=()):
        ...
        try:
            ...
            try:
                module = self._load_ptx_module(ptx_path)
            except Exception as e:
                kept_src, kept_ptx = self._preserve_failed_build(
                    src_path, ptx_path
                )
                raise RuntimeError(
                    f"{e}\n"
                    f"Failing source and PTX kept at:\n"
                    f"  {kept_src}\n"
                    f"  {kept_ptx}"
                ) from e
        finally:
    ...
    # CUjit_option values from the (ABI-stable) CUDA Driver API <cuda.h>.
    _CU_JIT_ERROR_LOG_BUFFER = 5
    _CU_JIT_ERROR_LOG_BUFFER_SIZE_BYTES = 6

    @staticmethod
    def _cuda_driver_lib():
        for libname in ("libcuda.so.1", "libcuda.so", "nvcuda.dll"):
            try:
                ...
                continue
        raise OSError(
            "Could not locate the CUDA driver library (libcuda) needed to "
            "load PTX compiled by clang."
        )

    def _load_ptx_module(self, ptx_path):
        # cupy's own module loading (cupy.RawModule / Module.load_file)
        # goes through the driver's plain cuModuleLoad, which on failure
        # only reports a generic "CUDA_ERROR_INVALID_PTX" with no further
        # detail. cuModuleLoadDataEx takes the same PTX but accepts a
        # JIT error-log buffer option, so the driver's own JIT compiler
        # (the same one behind the offline `ptxas` tool) writes its normal
        # diagnostic into it directly, in the same single load attempt,
        # rather than needing a second pass through an external tool.
        with open(ptx_path, "rb") as f:
            ptx_bytes = f.read()
        if not ptx_bytes.endswith(b"\0"):
            ptx_bytes += b"\0"

        cuda = self._cuda_driver_lib()

        log_size = 16384
        error_log = ctypes.create_string_buffer(log_size)
        option_keys = (ctypes.c_int * 2)(
            self._CU_JIT_ERROR_LOG_BUFFER_SIZE_BYTES,
            self._CU_JIT_ERROR_LOG_BUFFER,
        )
        option_values = (ctypes.c_void_p * 2)(
            log_size, ctypes.cast(error_log, ctypes.c_void_p).value
        )

        module_ptr = ctypes.c_void_p()
        status = cuda.cuModuleLoadDataEx(
            ctypes.byref(module_ptr), ptx_bytes, 2, option_keys, option_values
        )
        if status != 0:
            message = error_log.value.decode(errors="replace").strip()
            raise RuntimeError(
                f"CUDA driver failed to load the PTX generated by clang "
                f"(cuModuleLoadDataEx error {status}):\n"
                f"{message or '(driver gave no diagnostic message)'}"
            )

        module = cupy.cuda.function.Module()
        module.ptr = module_ptr.value
        return module

    def _preserve_failed_build(self, src_path, ptx_path):
        kept_dir = tempfile.mkdtemp(prefix="xobjects_clang_build_failure_")
        kept_src = os.path.join(kept_dir, "source.cu")
        kept_ptx = os.path.join(kept_dir, "source.ptx")
        shutil.copy(src_path, kept_src)
        shutil.copy(ptx_path, kept_ptx)
        return kept_src, kept_ptx
